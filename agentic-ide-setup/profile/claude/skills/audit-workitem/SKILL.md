...
Turn a work item into an implementation-ready execution plan. Optimize for fast agent sessions: small, targeted code changes with a short feedback loop, each completable in a single focused Claude Code session or subagent run. This skill produces analysis and a task backlog — do not implement code or alter the work item unless asked.
...
## Apply the fast-session test

Classify an item as fast-session-suitable only when it has all of these qualities:
...
Do not force-fit these into fast-session tasks: ambiguous stories, multi-service architecture, broad refactors, database/data migrations, security-sensitive changes, production remediation, cross-repository coordination, or investigations without a clear hypothesis. Mark each as **Long-horizon / human decision required** and explain the boundary that must be resolved first.

## Write the task breakdown

Order tasks by dependency. Make every fast-session task independently assignable and independently verifiable. Prefer vertical slices only when the slice remains narrow; otherwise stage the work through contracts, behavior, tests, and integration.
...
| Agent prompt | A self-contained prompt that tells the agent to inspect first, make only this change, run the named checks, and report files changed and remaining uncertainty. It must work when pasted unchanged into a fresh session with no access to this conversation. |
...
3. **Fast-session task backlog** — a dependency-ordered table using the required fields above. Prefix ready tasks with `Ready`.
4. **Keep out of the fast lane** — tasks requiring a longer-horizon agent or human judgment, with the reason and prerequisite to reconsider.
5. **Execution order** — the critical path, safely parallelizable tasks, and the final integration/verification task.

Use a high bar: a useful output creates tickets that can be pasted into a tracker and prompts that can be handed to an agent unchanged.
