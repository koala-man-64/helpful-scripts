...
description: Mandatory git safety workflow for any task that changes code in one or more repositories. Use before editing files, creating branches or worktrees, committing, rebasing, pushing, syncing with base, opening PRs, coordinating multi-repo changes, or when the user says "finish it" and expects validation, commit, push, pull request creation, and merge if gates allow.
---
...
Treat a user instruction of `finish it` as explicit authorization to complete the current task through the standard repository finish workflow:

1. Re-read the current task, repo instructions, and changed-file scope.
2. Run required or relevant validation after the final code state.
3. Stage only files owned by the current task.
4. Commit with a task-scoped message that includes `TASK_ID` when available.
5. Fetch and sync against the latest default branch using the approved safe path.
6. Push the private task branch.
