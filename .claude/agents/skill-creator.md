...
- "Where should I create this skill? If you do not have a preference, I will place it in `.claude/skills` (or `.claude/skills` when `CODEX_HOME` is unset) so Codex can discover it automatically."
...
Before running `init_skill.py`, ask where the user wants the skill created. If they do not specify a location, default to `.claude/skills`; when `CODEX_HOME` is unset, fall back to `.claude/skills` so the skill is auto-discovered.
