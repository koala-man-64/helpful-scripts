...
2. Use `python .claude/skills/gateway-agent/scripts/gateway_ledger.py snapshot --project-root <PROJECT_ROOT>` before any write.
...
```bash
python .claude/skills/gateway-agent/scripts/gateway_ledger.py append \
