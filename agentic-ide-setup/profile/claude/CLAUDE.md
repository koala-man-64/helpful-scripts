...
These are role mappings, not claims of equivalent capability to the Codex lanes. Model order is haiku < sonnet < opus; Fable is outside that order and is never a routed child. Each child must rank strictly below its parent, have bounded context (name an explicit `subagent_type`, never an unbounded fork), and own one non-overlapping deliverable. Integration decisions and final validation stay with the owner. Report each child's model, effort source, and routing reason.
